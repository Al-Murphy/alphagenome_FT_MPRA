"""
Finetune Enformer with MPRA head on LentiMPRA dataset using PyTorch Lightning.

This script uses the same optimal hyperparameters from AlphaGenome benchmarks
and the same dataset class (LentiMPRADataset) for fair comparison.

USAGE:
    python scripts/finetune_enformer_mpra.py --cell_type HepG2 --config configs/mpra_HepG2.json
"""
...
LentiMPRADataset = enf_utils.LentiMPRADatasetPyTorch

from enformer_pytorch import from_pretrained


class PyTorchLentiMPRADataset(torch.utils.data.Dataset):
    """PyTorch wrapper for LentiMPRADataset that converts numpy arrays to PyTorch tensors."""
    
    def __init__(self, dataset: LentiMPRADataset):
...
class LentiMPRADataModule(LightningDataModule):
    """PyTorch Lightning DataModule for LentiMPRA dataset."""
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == 'fit' or stage is None:
            self.train_dataset = PyTorchLentiMPRADataset(
                LentiMPRADataset(
            ...
            self.val_dataset = PyTorchLentiMPRADataset(
                LentiMPRADataset(
                    model=self.dummy_model,
                    path_to_data=self.data_path,
                    cell_type=self.cell_type,
        ...
        if stage == 'test' or stage is None:
            self.test_dataset = PyTorchLentiMPRADataset(
                LentiMPRADataset(
                    model=self.dummy_model,
                    path_to_data=self.data_path,
                    cell_type=self.cell_type,
...
class EnformerMPRALightning(pl.LightningModule):
    """PyTorch Lightning module for Enformer MPRA fine-tuning."""
    ...
    def __init__(
        ...
        # Create MPRA head (matches enf_utils.py design)
        self.model = EncoderMPRAHead(
            enformer=enformer,
            num_tracks=1,
        )
    ...
    def forward(self, seq_batch):
        ...
        """
        # EncoderMPRAHead expects (batch, length, 4) format
        freeze = (self.current_stage == 1)
        preds = self.model(seq_batch, freeze_enformer=freeze)
        return preds.squeeze(-1) if preds.dim() > 1 and preds.shape[-1] == 1 else preds
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_pearson', pearson, on_step=False, on_epoch=True, prog_bar=True)
        return loss
...
def create_dummy_model_for_dataset():
    """Create a dummy model for dataset initialization (needed for LentiMPRADataset).
...
def main():
    parser = argparse.ArgumentParser(
        description='Finetune Enformer with MPRA head on LentiMPRA dataset',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    
    parser.add_argument('--config', type=str, default=None, help='Path to JSON config file')
    parser.add_argument('--cell_type', type=str, default='HepG2', choices=['HepG2', 'K562', 'WTC11'])
    parser.add_argument('--data_path', type=str, default='./data/legnet_lentimpra')
    ...
    parser.add_argument('--wandb_project', type=str, default='enformer-mpra')
        ...
        if 'wandb' in config:
            ...
            if not args.no_wandb:
                ...
                if args.wandb_name is None:
                    args.wandb_name = wandb_config.get('wandb_name', f'enformer-{args.cell_type}')
        print("✓ Config loaded")
    
    if args.wandb_name is None:
        args.wandb_name = f'enformer-{args.cell_type}'
    
    # Create dummy model for dataset
    dummy_model = create_dummy_model_for_dataset()
    
    # Create data module
    print(f"\nSetting up LentiMPRA data module for {args.cell_type}...")
    ...
    # Model checkpointing
    checkpoint_dir = Path(args.checkpoint_dir) / args.cell_type / args.wandb_name
