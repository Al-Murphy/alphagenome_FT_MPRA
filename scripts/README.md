...
* **`test_cagi5_zero_shot_enformer_mpra.py`** - Zero-shot evaluation of fine-tuned Enformer MPRA models on CAGI5 benchmark.
...
* **`finetune_enformer_starrseq.py`** - Finetune Enformer model with DeepSTARR head on DeepSTARR dataset using PyTorch Lightning.
...
* **`test_ft_model_enformer_starrseq.py`** - Test fine-tuned Enformer DeepSTARR model on test data.
* **`test_mpralegnet.py`** - Test LegNet MPRA model on test data.
* **`compute_attributions_lentimpra.py`** - Compute attribution maps and sequence logos for lentiMPRA sequences using DeepSHAP, gradients, or gradient×input methods. Supports single sequences or top N sequences by activity.
